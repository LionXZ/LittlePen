...
def score_essay_4dimensions(essay_text: str, ocr_quality_note: str = "") -> dict:
    ...
    prompt = f"""你是儿童英文作文评分老师。请严格按以下标准对这篇小学生作文评分：
...
===== 评分流程 =====
1. 逐维度阅读作文，对照标准确定档位
2. 在档位分数范围内确定具体分数（取中间值：A=23, B=18, C=13, D=8, E=3；可根据实际情况微调 ±2）
3. total_score = 四维分数之和（不要算错）
4. overall_comment 用中文，先鼓励亮点，再提1个具体改进方向
...
{{
  ...
  "total_score": 62,
  "overall_comment": "你能写出完整的想法，真棒！试试在下一次作文中加入一些具体例子，比如你最喜欢的动物长什么样，它喜欢吃什么。"
}}
```"""
